...
{    
    ...
    if(secondsSince < 0)
        return NSLocalizedString(@"In The Future", nil);
...
- (NSString *)formatMinutesAgo:(NSTimeInterval)secondsSince
{
    ...
    if(minutesSince == 1)
        ...
        return [NSString stringWithFormat:NSLocalizedString(@"%d min ago", nil), minutesSince];
}
...
- (NSString *)formatAsToday:(NSTimeInterval)secondsSince
{
    ...
    //Handle Plural
    if(hoursSince == 1)
        return NSLocalizedString(@"1 hr ago", nil);
    else
        return [NSString stringWithFormat:NSLocalizedString(@"%d hrs ago", nil), hoursSince];
}
...
- (NSString *)formatAsYesterday
{
    ...
    return [NSString stringWithFormat:NSLocalizedString(@"Yes %@", nil), [dateFormatter stringFromDate:self]];
}
...
- (NSString *)formatAsLastMonth
{
    ...
    //Format
    [dateFormatter setDateFormat:NSLocalizedString(@"MMMM d 'at' h:mm a", nil)];
    return [dateFormatter stringFromDate:self];
}
